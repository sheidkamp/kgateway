...
// Package controlplanetls holds the body of the TestControlPlaneTLS e2e
// test.
...
// Run executes the TestControlPlaneTLS scenario against the Installation
// produced by the supplied factory. This test requires a dedicated
// installation with TLS enabled for xDS communication.
func Run(t *testing.T, factory e2e.InstallationFactory) {
	...
	testutils.Cleanup(t, func() {
		if err := testInstallation.ActionsProvider().Kubectl().Delete(cleanupCtx, []byte(nsYAML)); err != nil {
			t.Fatalf("failed to delete namespace: %v", err)
		}
	})
	err := testInstallation.ActionsProvider().Kubectl().Apply(t.Context(), []byte(nsYAML))
	if err != nil {
	...
	if err := testInstallation.ActionsProvider().Kubectl().Apply(t.Context(), []byte(secretYAML)); err != nil {
		t.Fatalf("failed to create TLS secret: %v", err)
	}
	...
	common.SetupBaseConfig(t.Context(), t, testInstallation.Underlying(), testdefaults.HttpbinManifest, gatewayManifest)
	common.SetupBaseGateway(t.Context(), t, testInstallation.Underlying(), types.NamespacedName{
		Namespace: "default",
		Name:      "gw",
	})
	...
	SuiteRunner().Run(t.Context(), t, testInstallation.Underlying())
}

// SuiteRunner returns the suite runner for the TestControlPlaneTLS scenario.
