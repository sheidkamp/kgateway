...
var _ = Describe("Deployer", func() {
	...
	Context("Port mapping enabled", func() {
		ports.Init(false)
		runDeployerTests(false)
	})

	Context("Port mapping disabled", func() {
		ports.Init(true)
		runDeployerTests(true)
	})
})
...
func runDeployerTests(usePortMapping bool) {
	maybeMappedPort80 := api.PortNumber(80)
	if usePortMapping {
		maybeMappedPort80 = api.PortNumber(8080)
	}
	...
	Describe("Deployer", func() {
		...
		Context("agentgateway", func() {
			...
			It("deploys agentgateway", func() {
				...
				Expect(svc.Spec.Ports[0].Port).To(Equal(int32(maybeMappedPort80)))
		...
		Context("Single gwc and gw", func() {
			...
			var (
						...
						Spec: gw2_v1alpha1.GatewayParametersSpec{
							Kube: &gw2_v1alpha1.KubernetesProxyConfig{
								...
								AiExtension: &gw2_v1alpha1.AiExtension{
									...
									Ports: []corev1.ContainerPort{{
										Name:          "foo",
										ContainerPort: int32(maybeMappedPort80),
									}},
								},
							},
						},
				...
				fullyDefinedGatewayParamsWithFloatingUserId = func() *gw2_v1alpha1.GatewayParameters {
					...
					if usePortMapping {
						// If port mapping is enabled, the sysctl shouldn't be set and the entire PodSecurityContext will be nil after unsetting RunAsUser
						params.Spec.Kube.PodTemplate.SecurityContext = nil
					} else {
			...
			fullyDefinedValidationWithoutRunAsUser := func(objs clientObjects, inp *input) error {
				...
				if gwUsesLowPorts && !usePortMapping {
			...
			DescribeTable("create and validate objs", func(inp *input, expected *expectedOutput) {
				...
				Entry("no port offset", defaultInput(), &expectedOutput{
					validationFunc: func(objs clientObjects, inp *input) error {
						...
						Expect(port.Port).To(Equal(int32(maybeMappedPort80)))
						Expect(port.TargetPort.String()).To(Equal(port.Name))
