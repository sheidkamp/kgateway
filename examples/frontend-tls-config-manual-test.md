...
kubectl expose deployment httpbin --port=8000 -n test-mtls
```

## Step 3: Configure Gateway with FrontendTLSConfig

### 3.1 Gateway with Default Validation (AllowValidOnly)

This configuration requires client certificates for all HTTPS listeners:
        ...
        validation:
          ...
          caCertificateRefs:
            - name: ca-cert-default
              kind: ConfigMap
              group: ""
            # Uncomment to test multiple CA certs:
...
### 3.2 Gateway with Per-Port Override
...
```bash
# Apply Gateway (choose one of the configurations above)
kubectl apply -f - <<EOF
# Paste one of the Gateway configurations above
EOF
...
## Step 4: Get Gateway External IP/Port
...
kubectl port-forward -n kgateway-system svc/kgateway 8443:8443 8444:8444
```
...
### 5.4 Test Multiple CA Certificates

If you configured multiple CA certificate refs, test with a client cert signed by the second CA:
...
openssl x509 -req -days 365 -in client2.csr -CA ca2-cert.pem -CAkey ca2-key.pem \
  -CAcreateserial -out client2-cert.pem

# Test with second CA's client cert
curl -v -k https://localhost:8443/get \
  --resolve example.com:8443:127.0.0.1 \
  -H "Host: example.com" \
...
Expected: HTTP 200 OK (if multiple CA refs are configured)
...
```bash
kubectl get gateway mtls-gateway -n test-mtls -o yaml
```
...
kubectl port-forward -n kgateway-system <envoy-pod> 15000:15000

# Check listener configuration
curl http://localhost:15000/config_dump | jq '.configs[2].dynamic_listeners[] | select(.name | contains("8443"))'
```
...
kubectl logs -n kgateway-system -l app=kgateway --tail=100 | grep -i "frontend\|mtls\|client.*cert"

# Check Envoy logs
kubectl logs -n kgateway-system -l app=envoy --tail=100 | grep -i "ssl\|tls\|certificate"
```
...
```bash
# Delete resources
kubectl delete gateway mtls-gateway -n test-mtls
